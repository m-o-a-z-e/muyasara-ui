...
export default function App() {
  ...
  const sendMessage = async (e) => {
    ...
    try {
      const response = await fetch('https://1be2-156-198-85-66.ngrok-free.app/webhook/0891b873-0b99-4e2a-a84e-5afcdfb1515e', {
        method: 'POST',
      ...
      setMessages([...newMessages, { role: 'assistant', content: data.output || 'عذراً، حدث خطأ في الرد.' }]);
    } catch (error) {
      setTimeout(() => {
        setMessages([...newMessages, { 
          role: 'assistant', 
          content: 'أكيد بخصوص استفسارك.. أنا ميسرة وجاهز أساعدك في كل لوائح المكتبة والأقسام! (جاري ربط الـ Webhook الفعلي).' 
        }]);
      }, 1000);
    } finally {
      setIsLoading(false);
  ...
  return (
    // ضفنا transition-colors عشان التغيير يكون ناعم، وكلاسات الـ dark للخلفية
    <div className="flex flex-col h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-300 font-sans" dir="rtl">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-lg tracking-wide text-white flex items-center gap-2">
              ميسرة <span className="text-xs bg-amber-500/20 text-amber-300 px-2 py-0.5 rounded-full border border-amber-500/30">الوكيل المرجعي</span>
            </h1>
            <p className="text-xs text-blue-300 dark:text-slate-400">مكتبة كلية الآداب - جامعة طنطا</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4">
        {messages.map((msg, idx) => (
          <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`p-4 rounded-2xl max-w-[85%] sm:max-w-[70%] text-sm leading-relaxed shadow-sm transition-colors duration-300 ${
              msg.role === 'user' 
